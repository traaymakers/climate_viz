...
t_data <- read.csv("Data/GLB.Ts+dSST.csv", skip = 1, na = "***") %>% 
  ...
  drop_na() %>% 
  mutate(month = factor(month, levels = month.abb)) %>% #change month to factor, otherwise it will be sorted alphabetically
  arrange(year, month) %>% 
  mutate(month_number = as.numeric(month),
         radius = t_diff + 1.5, #+1.5 to get rid of negative radius values
