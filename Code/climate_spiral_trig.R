...
gridlines <- tibble(theta = rep(2*pi*seq(0, 10, 0.01), each = 3),
  ...
  filter((line == "a" & theta >0.01*2*pi & theta < 0.99*2*pi) |
           ...
           (line == "c" & theta >0.06*2*pi & theta < 0.94*2*pi))
...
g <- t_data %>% 
  ...
  geom_path(data = gridlines %>%  filter(radius != radius_bump), aes(x=x, y=y, group = line),
               color="yellow",
               inherit.aes = FALSE) +
  geom_path(data = gridlines %>%  filter(radius == radius_bump), aes(x=x, y=y, group = line),
            color="green",
...
# ggsave("Figures/climate_spiral_trig.pdf", width = 4.5, height = 4.5, unit = "in", dpi = 300)
# ggsave("Figures/climate_spiral_trig.png", width = 4.155, height = 4.5, unit = "in", dpi = 300)
#        
animate(g, width = 4.5, height = 4.5, unit = "in", res = 300)
