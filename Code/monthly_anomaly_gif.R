...
t_data <- read.csv("Data/GLB.Ts+dSST.csv", skip = 1, na = "***") %>% 
  ...
  mutate(month = factor(month, levels = month.abb), #make months a factor to not have them sorted alphabetically by ggplot
...
p <- t_data %>% 
  ...
  labs(x = NULL,
       ...
       subtitle = "(Difference from 1980-2015 annual mean)b") + 
  theme(
...
p +
  ...
  geom_text(data = annotation, aes(x=4.6, y=1.1),
            label = "May 2022", hjust = 1)

# ggsave("Figures/monthly_anomaly.pdf", width = 15, height = 10, units = "cm")
# ggsave("Figures/monthly_anomaly.png", width = 15, height = 10, units = "cm")

p +
  geom_label(aes(x=7, y=0, label=year), fontface = "bold", label.size = 0) + 
  transition_manual(year, cumulative = TRUE)

animate(p, width = 15, height = 10, units = "cm", res = 300)
